...
A server reconstructs the intended target URI as an "effective request URI" to properly service the request. This reconstruction involves both the server’s local configuration and information communicated in the request-target, Host header field, and connection context.

If the request-target is in absolute-form (proxy), the effective request URI is the same as the request-target.  
Otherwise, the effective request URI is constructed as follows:
- the scheme is "http".
- Authority:
	- If the server’s configuration provides a fixed URI authority component, that authority is used for the effective request URI. 
	<!-- - If not, then if the request-target is in authority-form, the effective request URI’s authority component is the same as the request-target. - **OSEF** -->
	...
	- Otherwise, the authority component is assigned the default name configured for the server and, if the connection’s incoming TCP port number differs from the default port, the incoming port number is appended to the authority component.
<!-- - If the request-target is in authority-form or asterisk-form, the effective request URI’s combined path and query component is empty. Otherwise, the combined path and query component is the same as the request-target.  - **OSEF** -->
...
### Message Forwarding
An intermediary not acting as a tunnel MUST
- implement the Connection header field,
- exclude fields from being forwarded that are only intended for the incoming connection.

### Via
See section 5.7.1 of RFC7230

### Transformations
**OSEF**

### Connection Management
A proxy or gateway MUST remove or replace any received connection options before forwarding the message.
...
```
Connection options are case-insensitive. The connection options do not always correspond to a header field present in the message, since a connection-specific header field might not be needed if there are no parameters associated with a connection option.

### _Close_
The "close" connection option is defined for a sender to signal that this connection will be closed after completion of the response.
```
`Connection`: close
```
...
### Persistence
- If the "close" connection option is present, the connection will not persist;
- If the received protocol is HTTP/1.1 (or later), the connection will persist;
- If the received protocol is HTTP/1.0, the "keep-alive" connection option is present,  the connection will persist after the current response;
- The connection will close after the current response.

### Retrying Requests
Connections can be closed at any time. Implementations ought to anticipate the need to recover from
...
### Tear-down
- A server that receives a "close" connection option MUST initiate a close of the connection after it sends the final response.
...
1. First, the server performs a half-close by closing only the write side of the read/write connection.
2. The server then continues to read from the connection until it receives
	- a corresponding close by the client,
...
3. Finally, the server fully closes the connection.
